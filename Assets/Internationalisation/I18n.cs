...
class I18n
{
    ...
    /// </summary>
    /// 
    public static void LoadLanguageFromSystemLanguage()
    {
        if (Fields == null)
    ...
    public static void LoadLanguageFromGameData()
    {
        if (Fields == null)
            Fields = new Dictionary<string, string>();

        Fields.Clear();
        string lang = Get2LetterISOCodeFromGameData().ToLower();
        Debug.Log($"{lang}");
        //lang = "de";
        var textAsset = Resources.Load(@"I18n/" + lang); //no .txt needed
        string allTexts = "";
    ...
    public static string Get2LetterISOCodeFromGameData()
    {
